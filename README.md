...
```markdown
### Анти-галлюцинации (как мы боремся с выдумками LLM)

1. **LLM не считает числа** — все метрики (min, max, mean) берутся из кода
2. **Строгий контекст** — LLM получает только агрегаты, а не сырые данные
3. **Явный запрет** — в промпте указано: *"Do not invent numbers"*
4. **Проверяемость** — каждая цифра в LLM-сводке совпадает с метриками из кода
5. **Логирование** — каждый запрос к LLM фиксируется в `docs/LLM_Usage_Log.md`

**Результат:** LLM выступает только как "пишущий и объясняющий слой", а не как калькулятор.
```
